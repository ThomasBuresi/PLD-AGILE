...
public class DijkstraGraph implements Graph {
	...
	/**
	 * Returns all shortest paths between pairs of interest points (depot, pickup
	 * and delivery points)
	 * 
	 ...
	 * @return the final state of the shortest path search from origin to
	 *         destination
	 */
	...
	/**
	 * Creates a graph to be used later by the TSP calculator, based on a specific
	 * list of requests
	 */
	private void fillGraph() {
		edges = new float[1 + 2 * requestList.getListRequests().size()][1 + 2 * requestList.getListRequests().size()];
		segmentPaths = new List[1 + 2 * requestList.getListRequests().size()][1
				+ 2 * requestList.getListRequests().size()];
			...
			for (int j = 0; j < 1 + 2 * requestNb; j++) {
				if (i == j) {
					...
					if (j != -1) {
						...
						destination = (j % 2 == 0) ? destinationRequest.getPickupAddress()
								: destinationRequest.getDeliveryAddress();
					} else {
					...
					if (finalState == null) {
						...
						segmentPaths[i][j] = finalState.getAllPreviousSegments();
					}
				}
			}
		}
